...
let HST;
let HST2;
const contract2test = BNB;
const contract_name = 'BNB';
const contract_symbol = "BNB";
...
contract(contract_name, (accounts) => {
  beforeEach(async () => {
    HST = await contract2test.new(10000,"BNB",18,"BNB",{ "from": accounts[0] });
    HST2 = await contract2test.new(maxtokens, 'XYZ',18,'BNB',{ "from": accounts[0] });
  });
