...
let HST;
let HST2;
const contract2test = BitAseanToken;
const contract_name = 'BitAseanToken';
...
contract(contract_name, (accounts) => {
  beforeEach(async () => {
    HST = await contract2test.new(10000,"BitAseanToken",10,"XYZ",{ "from": accounts[0] });
    HST2 = await contract2test.new(maxtokens,'XYZ',1,'XYZ',{ "from": accounts[0] });
  });
