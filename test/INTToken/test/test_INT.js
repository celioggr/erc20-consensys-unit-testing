...
let HST;
let HST2;
const contract2test = INT;
const contract_name = 'Internet Node Token';
const contract_symbol = "INT";
...
contract(contract_name, (accounts) => {
  beforeEach(async () => {
    HST = await contract2test.new({"from": accounts[0]})
    HST2 = await contract2test.new({ "from": accounts[0] });
  });
  ...
  */
  it('transfers: should transfer 10000 to accounts[1] with accounts[0] having 10000', async () => {
    await HST.transfer(accounts[1], 1000000000000000, { from: accounts[0] });
    const balance = await HST.balanceOf(accounts[1]);
    assert.strictEqual(balance.toNumber(), 1000000000000000);
  });
